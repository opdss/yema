...
echo Running "go $@"
exec go "$1" -ldflags \
  "-w -s -X go-walle/app/version.buildTimestamp=$TIMESTAMP
   -X go-walle/app/version.buildCommitHash=$COMMIT
   -X go-walle/app/version.buildVersion=$VERSION
   -X go-walle/app/version.buildRelease=$RELEASE" "${@:2}"
